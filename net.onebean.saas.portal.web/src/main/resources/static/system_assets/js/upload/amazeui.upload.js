...
(function($) {
  ...
  AmazeuiUpload.prototype._pasteHand = function(_this, e) {
      ...
      for (var i = 0; i < clipboard.items.length; i++) {
          var temp = clipboard.items[i].getAsFile();
          tempArr.push(temp);
      };
      _this._handFiles(tempArr);
      e.preventDefault();
      e.stopPropagation();
    }
  ...
  AmazeuiUpload.prototype._handFiles = function(files) {
      ...
      for (var i = 0; i < files.length; i++) {
          if(files[i] == null ||(typeof(_this.amazeuiUploadDelegateEvent.fileMap[files[i].name]) != 'undefined'
              && _this.amazeuiUploadDelegateEvent.fileMap[files[i].name] == files[i].size)){
              return
          }else{
              ...
              for (var i = 0; i < files.length; i++) {
                  ...
                  var tempFileInfo = new Object();
                  tempFileInfo.name = _this.thisFile.name;
                  tempFileInfo.size = _this.thisFile.size;
                  _this.amazeuiUploadDelegateEvent.fileMap[tempFileInfo.name]=tempFileInfo.size;
              };
          }
      }
    }
